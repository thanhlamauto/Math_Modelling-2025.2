...
For each parameter setting, independent Monte Carlo trials are averaged. The reference paper uses 1000 trials for its main figures. Our generated figures use \texttt{MC\_RUNS=200} for computational efficiency, and the script can be rerun with \texttt{MC\_RUNS=1000 python3 simulate\_superspreaders.py} to match the paper's averaging level more closely. Shaded bands in the dynamic plots represent the standard error across trials.
...
All figures in this section are generated by \texttt{simulate\_superspreaders.py}. The default number of Monte Carlo trials is controlled by \texttt{MC\_RUNS}. This keeps the submitted code practical to run while preserving the same model, parameters, and measured quantities as the reference.
